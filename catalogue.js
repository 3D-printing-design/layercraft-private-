...
router.post('/', requireAuth, async (req, res) => {
  try {
    const { name, category, description, base_price, materials, colours, shape, tag } = req.body;
    if (!name || !base_price) return res.status(400).json({ error: 'Name and price required' });

    const { data, error } = await supabase
      .from('catalogue')
      .insert({ name, category, description, base_price, materials, colours, shape, tag, active: true })
      .select()
      .single();
    if (error) throw error;
